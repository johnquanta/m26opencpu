...
static void Fota_Start( u8 *BoardID )
{
	...
	MY_MEMCPY(apnCfg.apnName,	APN, Ql_strlen(APN));
	MY_MEMCPY(apnCfg.apnUserId, USERID, Ql_strlen(USERID));
	MY_MEMCPY(apnCfg.apnPasswd, PASSWD, Ql_strlen(PASSWD));
...
{ 
    ...
    { 
		...
		case UP_GET_FILE_OK: 
			Ql_Debug_Trace("<-- Fota Upgrading... -->\r\n"); 
			break; 
		case UP_FOTAINITFAIL: 
		case UP_URLDECODEFAIL: 
		case UP_UPGRADFAILED: 
			Ql_Debug_Trace("<-- Fota Upgrade failed!! -->\r\n"); 
			Ql_Reset(0);
			return FALSE; 
...
{
	if(  Ql_strstr( FOTA.Upgrading,  UPGRADING_YES )  )
	{	MY_MEMSET( FOTA.Upgrading, 0, sizeof(FOTA.Upgrading)  );
	MY_DEBUG( "Start Upgrading...\r\n" );
	Fota_Start( BoardID );
}
		...
		MY_DEBUG( "can not Start Upgrading...\r\n" );
	   return;
}
