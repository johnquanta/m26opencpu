...
void send_gps_data()
{
	u8 buf_temp[64]={0};
		Ql_memset(buf_temp,0,sizeof(buf_temp));
			...
			send_toserver_data((u8*)buf_temp,Ql_strlen(buf_temp),0x82);

}
void send_toserver_data(u8 *Data,u32 len,u8 command)
{
 u8 *send_data;
 u16 check=0;
 send_data[0]=0xd6;
 ...
 send_data[4]=5;
 send_data[5]=command;
  Ql_memcpy( send_data+6, Data, len);
...
DataEncrypt( send_data, len+6, send_data );
DepositSendingData( send_data, len+6 );
