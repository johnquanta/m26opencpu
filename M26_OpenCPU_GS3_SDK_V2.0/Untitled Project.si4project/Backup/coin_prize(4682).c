...
if( g_CoinIn_NONC == CLOSE )	//投币常闭
 {
	 ...
	 {					 
		 ...
		 #if  COIN_TEST_DEBUG  
		 MY_DEBUG( "Close:g_coinInPulseWidthTime==%dms\r\n",g_prizePulseWidthTime*COIN_IN_TIMER_PERIOD );
		 #endif
	 }
 }
else 	//投币常开
...
if( g_prizePulseStartFlag )  //有脉冲，开始计时。
{
	...
	if( g_prizePulseWidthTime*COIN_IN_TIMER_PERIOD > device.commTaskData.ioPara.plusePara[PLUSE_IN_W_MAX0] )  ////超出常开常闭检测时间。
	{
...
else	   //脉冲结束。
{
	...
	if(    g_prizePulseWidthTime*COIN_IN_TIMER_PERIOD > device.commTaskData.ioPara.plusePara[PLUSE_IN_W_MIN0] 
		&& g_prizePulseWidthTime*COIN_IN_TIMER_PERIOD < device.commTaskData.ioPara.plusePara[PLUSE_IN_W_MAX0] )
	{ 
		g_prizePulseIntervalTime++;   //
	}
	if( g_prizePulseIntervalTime*COIN_IN_TIMER_PERIOD > device.commTaskData.ioPara.plusePara[PLUSE_IN_W_MIN0] )
	{
				 FLAG.CoinPlusCount=g_prizePulseWidthTime*COIN_IN_TIMER_PERIOD;
