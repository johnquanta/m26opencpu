...
static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "wmq.weimaqi.net";
static u32 m_SrvPort = 9136;
...
static void Callback_Timer(u32 timerId, void* param)
{
    ...
    else if (TCP_TIMER_ID == timerId)
    {
        ...
        switch (m_tcp_state)
        {
            ...
            {
                s32 simStat = 0;
                RIL_SIM_GetSimState(&simStat); //检测SIM卡状态
            ...
            case STATE_NW_QUERY_STATE:
            {
                ...
                ret = RIL_NW_GetGSMState(&creg);//GPS网络注册状态
                ret = RIL_NW_GetGPRSState(&cgreg);//GPRS网络注册状态
