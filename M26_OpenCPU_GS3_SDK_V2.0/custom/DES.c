...
static void rotatel(bool *in, int len, int loop,bool *tmp);//????
 //????????
...
void des_setkey(const char key[8])
{
    ...
    for(i=0;i<16;i++)
    {
       rotatel(kl,28,loop_table[i],tmp);
       rotatel(kr,28,loop_table[i],tmp);
...
void rotatel(bool *in,int len,int loop, bool *tmp)
{
    //static bool tmp[256];
	memcpy(tmp,in,loop);
