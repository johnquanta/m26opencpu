 ...
 ****************************************************************************/
#if 1//20171226//def __EXAMPLE_SMS__
#include "ril.h"
#include "ril_util.h"
#include "ril_sms.h"
...
#define APP_DEBUG(FORMAT,...) {\
    ...
    Ql_sprintf(DBG_BUFFER,"%s(%d):" FORMAT, __func__, __LINE__,##__VA_ARGS__); \
...
void SMS_TextMode_Send(void)
{
    s32 iResult;
    u32 nMsgRef;
    char strPhNum[] = "+8613660510453\0";//20171226//"+8613651951196\0";
...
static void Hdlr_RecvNewSMS(u32 nIndex, bool bAutoReply)
{
    ...
    APP_DEBUG("<-- RIL_SMS_ReadSMS_Text.,nIndex:%u -->\r\n",nIndex);//20171226//
 ...
 *****************************************************************************/
 ...
 #endif
void module_sms_proc_main_task2(s32 iTaskID)//20171226//void proc_main_task(s32 iTaskID)
{
    s32 iResult = 0;
    ST_MSG taskMsg;
    ...
    //20171226//APP_DEBUG("OpenCPU: SMS Example\r\n");
    ...
    while (TRUE) 
    {
        ...
        //20171226//Ql_memset(&taskMsg, 0x0, sizeof(ST_MSG));
        //20171226//Ql_OS_GetMessage(&taskMsg);
        switch (taskMsg.message)
        {
        case MSG_ID_RIL_READY:
            {
                //20171226//APP_DEBUG("<-- RIL is ready -->\r\n");
                //20171226//Ql_RIL_Initialize(); // MUST call this function

                //20171226//for(i = 0; i < CON_SMS_BUF_MAX_CNT; i++)
                //20171226//{
                    //20171226//ConSMSBuf_ResetCtx(g_asConSMSBuf,CON_SMS_BUF_MAX_CNT,i);
                //20171226//}

				//20171226//smsIni();//20171226//
